...
export function AppSidebar({ collapsed, setCollapsed }: { collapsed: boolean; setCollapsed: (collapsed: boolean) => void }) {
  ...
  return (
    ...
    >
      {/* Logo */}
      <div className="flex h-16 items-center gap-3 border-b border-border px-4 cursor-pointer" onClick={() => setCollapsed(!collapsed)}>
        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-primary">
          <Layers className="h-5 w-5 text-primary-foreground" />
        </div>
        <AnimatePresence>
          {!(collapsed && !isMdAndSmaller) && ( // show title always on mobile, and when not collapsed on desktop
            <motion.span
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
